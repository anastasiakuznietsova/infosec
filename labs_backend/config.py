PRNG_COMPARISON_MODULE = 2 ** 21 - 1
PRNG_MULTIPLIER = 8 ** 3
PRNG_INCREMENT = 144
PRNG_SEED = 3

HASH_MD_BUFFER = [0x67452301,0xEFCDAB89,0x98BADCFE,0x10325476]
HASH_SIN_TABLE = [0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501, 0x698098d8,
                 0x8b44f7af, 0xffff5bb1, 0x895cd7be, 0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
                 0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa, 0xd62f105d, 0x2441453, 0xd8a1e681, 0xe7d3fbc8, 0x21e1cde6,
                 0xc33707d6, 0xf4d50d87, 0x455a14ed, 0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
                 0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c, 0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70, 0x289b7ec6,
                 0xeaa127fa, 0xd4ef3085, 0x4881d05, 0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
                 0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039, 0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1, 0x6fa87e4f,
                 0xfe2ce6e0, 0xa3014314, 0x4e0811a1, 0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391]
HASH_CYCLIC_SHIFT = [7, 12, 17, 22] * 4 + [5, 9, 14, 20] * 4 + [4, 11, 16, 23] * 4 + [6, 10, 15, 21] * 4


WORD_LENGTH_RC5 = 64
ROUNDS_AMOUNT_RC5 = 20
KEY_LENGTH_RC5 = 16
MASK_RC5 = 0xFFFFFFFFFFFFFFFF
P_RC5 = 0xb7e151628aed2a6b
Q_RC5 = 0x9e3779b97f4a7c15
TO_BYTES_LENGTH_RC5 = 8
CHECK_HEADER_RC5 = b"BABABOI"

PUBLIC_EXPONENT_RSA = 65537
KEY_SIZE_RSA = 2048

ERROR_MSGS_BACKEND = {
    'INCORRECT_PASSWORD_FORMAT' : "Incorrect password format",
    "NO_INPUT":"Input wasn’t provided",
    "DAMAGED_OR_EMPTY_FILE": "File is damaged or empty",
}

MEDIA_TYPES = {
    'BYTES':"application/octet-stream"
}